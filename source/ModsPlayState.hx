...
class ModsPlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (isStoryMode)
		{
			detailsText = "Story Mode: " + ModsWeekData.getCurrentWeek().weekName;
		}
		else
		{
		...
		curStage = ModsPlayState.SONG.stage;
		trace('stage is: ' + curStage);
		if(ModsPlayState.SONG.stage == null || ModsPlayState.SONG.stage.length < 1) {
			switch (songName)
			{
		...
		var doPush:Bool = false;
		var luaFile:String = 'stages/mods/' + curStage + '.lua';
		...
		var file:String = Paths.json('mods/' + songName + '/dialogue'); //Checks for json/Psych Engine dialogue
		...
		var doPush:Bool = false;
		var luaFile:String = 'data/mods/' + Paths.formatToSongPath(SONG.song) + '/script.lua';
	    luaFile = Paths.getPreloadPath(luaFile);
		    if(OpenFlAssets.exists(luaFile)) {
				doPush = true;
			}
	...
	function startSong():Void
	{
		...
		FlxG.sound.playMusic(Paths.inst(ModsPlayState.SONG.song), 1, false);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices)
			vocals = new FlxSound().loadEmbedded(Paths.voices(ModsPlayState.SONG.song));
		else
			vocals = new FlxSound();

		FlxG.sound.list.add(vocals);
		FlxG.sound.list.add(new FlxSound().loadEmbedded(Paths.inst(ModsPlayState.SONG.song)));
		...
		var songName:String = Paths.formatToSongPath(SONG.song);
		var file:String = Paths.json('mods/' + songName + '/events');
		if (OpenFlAssets.exists(file)) {
	...
	override public function update(elapsed:Float)
	{
		...
		setOnLuas('botPlay', ModsPlayState.cpuControlled);
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					// if ()
					if(!usedPractice) {
						StoryMenuState.weekCompleted.set(ModsWeekData.weeksList[storyWeek], true);

						if (SONG.validScore)
						{
							Highscore.saveWeekScore(ModsWeekData.getWeekFileName(), campaignScore, storyDifficulty);
						}
				...
				else
				{
					...
					trace(Paths.formatToSongPath(ModsPlayState.storyPlaylist[0]) + difficulty);
					...
					ModsPlayState.SONG = Song.loadFromJson(ModsPlayState.storyPlaylist[0] + difficulty, ModsPlayState.storyPlaylist[0]);
					FlxG.sound.music.stop();
					...
					if(winterHorrorlandNext) {
						new FlxTimer().start(1.5, function(tmr:FlxTimer) {
							...
							LoadingState.loadAndSwitchState(new ModsPlayState());
						});
					} else {
						cancelFadeTween();
						//resetSpriteCache = true;
			...
			else
			{
				...
				MusicBeatState.switchState(new ModsFreeModsPlayState());
				FlxG.sound.playMusic(Paths.music('freakyMenu'));
				usedPractice = false;
				changedDifficulty = false;
				cpuControlled = false;
	...
	public function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		...
		if(ModsPlayState.SONG.splashSkin != null && ModsPlayState.SONG.splashSkin.length > 0) skin = ModsPlayState.SONG.splashSkin;
	...
	override function beatHit()
	{
		...
		if (generatedMusic && ModsPlayState.SONG.notes[Std.int(curStep / 16)] != null && !endingSong && !isCameraOnForcedPos)
		{
			...
			if(!Achievements.isAchievementUnlocked(achievementName)) {
				...
				switch(achievementName)
				{
					case 'week6_nomiss':
						...
						{
							var weekName:String = ModsWeekData.getWeekFileName();
