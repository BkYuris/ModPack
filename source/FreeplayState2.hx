...
class FreeplayState2 extends MusicBeatState
{
	...
	override function create()
	{
		...
		Paths.destroyLoadedImages();
 		#end	
		WeekData2.reloadWeekFiles(false);
		#if desktop
		// Updating Discord Rich Presence
		DiscordClient.changePresence("In the Menus", null);
		#end

		for (i in 0...WeekData2.weeksList.length) {
			var leWeek:WeekData2 = WeekData2.weeksLoaded.get(WeekData2.weeksList[i]);
			...
			WeekData2.setDirectoryFromWeek(leWeek);
		...
		WeekData2.setDirectoryFromWeek();
	...
	override function update(elapsed:Float)
	{
		...
		else #end if (accepted)
		{
			...
			trace('CURRENT WEEK: ' + WeekData2.getWeekFileName());
			if(colorTween != null) {
				colorTween.cancel();
			}
