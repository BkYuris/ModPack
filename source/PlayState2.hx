...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (isStoryMode)
		{
			detailsText = "Story Mode: " + WeekData2.getCurrentWeek().weekName;
		}
		else
		{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					// if ()
					if(!usedPractice) {
						StoryMenuState.weekCompleted.set(WeekData2.weeksList[storyWeek], true);

						if (SONG.validScore)
						{
							Highscore.saveWeekScore(WeekData2.getWeekFileName(), campaignScore, storyDifficulty);
						}
			...
			if(!Achievements.isAchievementUnlocked(achievementName)) {
				...
				switch(achievementName)
				{
					case 'week6_nomiss':
						...
						{
							var weekName:String = WeekData2.getWeekFileName();
